...
class Tpl(Sequence):
    async def init(self):
        ...
        if hlcc_available():
            ...
            self.ccs_uri = cons.get_uri(f"telifsim")
        ...
        if hlcc_available():
            self.data_sources.append(MetadataSource(name=self.ccs_name,
                                                    rr_uri=f"{self.ccs_uri}/telifsim/MetaDaq"))
    ...
    def create(*args, **kw):
        ...
        s = Sequence.create(a.init, a.setup, p, name="tdm_img_obs", **kw)
        return s
